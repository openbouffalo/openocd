...
static int bl602_flash_gpio_init(struct flash_bank *bank)
{
	...
	if (part_info->series == BFLB_SERIES_BL702 ||
		...
		uint32_t sw_usage;
		retval = target_read_u32(target, 0x40007010, &sw_usage);
...
static int bl602_flash_init(struct flash_bank *bank)
{
	...
	if (part_info->series == BFLB_SERIES_BL602 ||
		part_info->series == BFLB_SERIES_BL702) {
	...
	} else if (part_info->series == BFLB_SERIES_BL702L) {
		/* The structure of SF_Ctrl_Cfg_Type is same as on BL602 and BL702, but
		 * BL702L's BootROM was compiled without -fshort-enums flag.
...
static void bl602_free_driver_priv(struct flash_bank *bank)
{
...
const struct flash_driver bl602_flash = {
	...
	.free_driver_priv = bl602_free_driver_priv
};
