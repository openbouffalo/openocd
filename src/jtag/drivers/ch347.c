...
#define TCK_H	BIT(0)
#define TCK_L	0
// For TRST: CH347F uses GPIO3, while CH347T uses GPIO5
#define TRST_H_CH347F BIT(3)
#define TRST_L_CH347F 0
#define TRST_H_CH347T BIT(5)
#define TRST_L_CH347T 0
#define LED_ON 1
#define LED_OFF	0
...
static uint16_t custom_ch347_vids[] = {0, 0};
static uint16_t custom_ch347_pids[] = {0, 0};
 ...
 * @return ERROR_OK at success
 */
static int ch347_scratchpad_add_move_state(tap_state_t state, int skip)
{
	uint8_t tms_scan;
	int tms_len;
 ...
 * @return ERROR_OK at success
 */
static int ch347_scratchpad_add_run_test(int cycles, tap_state_t state)
{
...
static int ch347_reset(int trst, int srst)
{
	...
	if (ch347.chip_variant == CH347T)
		ch347.trst_pin = trst ? TRST_L_CH347T : TRST_H_CH347T;
	else
		ch347.trst_pin = trst ? TRST_L_CH347F : TRST_H_CH347F;
...
static int ch347_speed_get_index(int khz, int *speed_idx)
{
	...
	if (swd_mode) {
		// Don't allow too low clk speeds: packet processing is limited to ~16 msec
...
COMMAND_HANDLER(ch347_handle_vid_pid_command)
{
	...
	COMMAND_PARSE_NUMBER(u16, CMD_ARGV[0], custom_ch347_vids[0]);
	COMMAND_PARSE_NUMBER(u16, CMD_ARGV[1], custom_ch347_pids[0]);
...
static int ch347_init(void)
{
	...
	ch347.tms_pin = TMS_H;
	ch347.tdi_pin = TDI_L;
	ch347.trst_pin = ch347.chip_variant == CH347T ? TRST_H_CH347T : TRST_H_CH347F;
...
static int ch347_swd_send_idle(uint32_t ap_delay_clk)
{
	...
	if (ch347_swd_context.sent_cmd_count) {
		...
		unsigned int expected_recv_len = ch347_swd_context.need_recv_len + 1;
					// 0xA1
		if (expected_total_clk * ch347_swd_context.clk_divisor > CH347_MAX_PROCESSING_US
